...
def fnget_sdql_data(new_querylist):
    ...
    for nq in new_querylist:
        ...
        ################# The 4 lines below will convert tp( tpp(, tp5( etc. Letting the API handle instead
            ...
            if 'money line ' in s:
                for l in money_lines:
                    if l in s and l+'[' not in s:
                        if s[len(s)-1:len(s)]==',':
                            s=s[:-1].replace(l,l+'[-1]')+as_words+','
                        else:
                            s=s.replace(l,l+'[-1]')+as_words

            else:
                if 'line' in s and 'odds' in s:
                    for l in lines:
                        if l in s and 'odds' in l and l+'[' not in s:
                            if s[len(s)-1:len(s)]==',':
                                s=s[:-1].replace(l,l+'[-1]')+as_words+','
                            else:
                                s=s.replace(l,l+'[-1]')+as_words
...
for sc_key,sc_value in shortcuts.items():
    ...
    if sc_key[:2]=='p:':
        newshortcuts.update({sc_key.replace('p:','P:').replace('po:','Po:').replace('to:','Po:'): sc_value.replace('p:','P:').replace('po:','Po:').replace('to:','Po:')})
    if sc_key[:3]=='op:':
